...
void GameMenu::setupGameSetupScreen() {
    ...
    if (selectedTab == 0) {
        ...
        labels.emplace_back(gameConfig.spidersMode
                                ? "Spiders: ON"
                                : "Spiders: OFF");
        labels.emplace_back(gameConfig.dementiaMode
                                ? "Dementia: ON"
                                : "Dementia: OFF");
        labels.emplace_back(gameConfig.endermanMode
                                ? "Enderman: ON"
                                : "Enderman: OFF");
    }
...
void GameMenu::setupOptionsScreen() {
    ...
    if (pendingFullscreen) {
        buttonManager.setButtonEnabled(
            3, false); // Disable Resolution slider if fullscreen
    }
    ...
    if (const auto *mouseButton = event.getIf<sf::Event::MouseButtonPressed>()) {
        ...
        if (menuState == MenuState::Options) {
            ...
            if (buttonManager.getButtonCount() > 0) {
                if (buttonManager.getButtonCount() >
                    5) {
                    // Ensure we have options loaded
                    // Master
...
void GameMenu::handleOptionsClick(int buttonIndex,
    ...
    if (buttonIndex <= 3) {
        ...
        buttonManager.setButtonText(4, pendingFullscreen
                                           ? "Fullscreen: ON"
                                           : "Fullscreen: OFF");
        buttonManager.setButtonEnabled(
            3, !pendingFullscreen); // Disable resolution slider
...
void GameMenu::drawOverlay(sf::RenderWindow &window) const {
    ...
    footerSep.setPosition({
        0.f, static_cast<float>(window.getSize().y) -
