...
bool Grid::handleInput(const sf::Event &event, const sf::RenderWindow &window) {
    if (gameMode) {
...
void Grid::healWebs() {
    ...
    for (size_t i = 0; i < rowHints.size(); ++i) {
        for (size_t j = 0; j < rowHints[i].size(); ++j) {
            if (hints.isWebbed(true, i, j)) {
                int health = hints.getWebHealth(true, i, j);
                if (health < 10) {
                    hints.setWebHealth(true, i, j, 10); // Fully heal
                }
            }
        }
    }
    ...
    for (size_t j = 0; j < colHints.size(); ++j) {
        for (size_t i = 0; i < colHints[j].size(); ++i) {
            if (hints.isWebbed(false, j, i)) {
                int health = hints.getWebHealth(false, j, i);
                if (health < 10) {
                    hints.setWebHealth(false, j, i, 10);
                }
            }
        }
    }
...
void Grid::setRenderer(const GridRenderer *r) {
    if (gameMode)
