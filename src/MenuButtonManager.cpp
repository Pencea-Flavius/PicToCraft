...
void MenuButtonManager::layoutGameSetup(
    ...
    const sf::Texture &inactiveButtonTexture) {
  ...
  for (size_t i = 2; i < buttons.size() - 2; i++) {
    ...
    bool shouldDisable = (selectedTab == 1 && i == 2 && !isTimeModeAvailable);
    ...
    buttons[i]->update(scale, centerX, contentY + (i - 2) * spacing, 1.2f, 1.0f,
                       mousePos);
  }
...
void MenuButtonManager::update(const sf::Vector2f &mousePos) {
  for (auto &button : buttons) {
    button->update(1.0f, 0.0f, 0.0f, 1.0f, 1.0f, mousePos);
  }
}

void MenuButtonManager::draw(sf::RenderWindow &window) {
  for (auto &button : buttons) {
    button->draw(window);
  }
}
