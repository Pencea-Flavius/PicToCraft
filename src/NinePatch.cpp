...
void NinePatch::updateLayout() {
  ...
  int edgeWidth = texWidth - 2 * m_cornerSize;
  int edgeHeight = texHeight - 2 * m_cornerSize;
  float baseScale = (m_edgeThickness > 0)
                        ? static_cast<float>(m_edgeThickness) /
                              static_cast<float>(m_cornerSize)
                        : 1.0f;
  float finalPatchScale = baseScale * m_patchScale;

  float scaledCornerSize = static_cast<float>(m_cornerSize) * finalPatchScale;

  // Positions
  float x[4] = {0.f, scaledCornerSize, targetWidth - scaledCornerSize,
                targetWidth};
  float y[4] = {0.f, scaledCornerSize, targetHeight - scaledCornerSize,
                targetHeight};
  ...
  m_sprites[1].setScale(
      {(x[2] - x[1]) / static_cast<float>(edgeWidth), finalPatchScale});
  ...
  m_sprites[3].setScale(
      {finalPatchScale, (y[2] - y[1]) / static_cast<float>(edgeHeight)});

  // 4: Center
  m_sprites[4].setPosition({x[1], y[1]});
  m_sprites[4].setScale({(x[2] - x[1]) / static_cast<float>(edgeWidth),
                         (y[2] - y[1]) / static_cast<float>(edgeHeight)});

  // 5: Right
  m_sprites[5].setPosition({x[2], y[1]});
  m_sprites[5].setScale(
      {finalPatchScale, (y[2] - y[1]) / static_cast<float>(edgeHeight)});
